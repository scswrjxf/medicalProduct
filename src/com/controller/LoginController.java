...
public class LoginController{
	...
	public ModelAndView index(HttpSession session) { 
		...
		//热卖商品
		Goods hot=cartService.hotGoods();
		mv.addObject("hot",hot);
		//最新评论
		List<Comment> commentnew=cartService.findCommentFive();
		mv.addObject("commentnew",commentnew);
		//全部商品
	...
	public String AddCart(@PathVariable Integer goodsId,Model model,HttpSession session ) {
		...
		// 判断购物车商品是否已经存在
		Cart result = cartService.findCartByGoodsName(goods.getGoodsName(),loginer.getUserId());
		...
		cartService.addNewCart(loginer.getUserId(),goods.getGoodsName(),goodsId,goods.getGoodsPrice(),goods.getGoodsPhoto());
		}
		//在model中设置一个cart值
	...
	@ResponseBody
	public Object updateNumber(Integer cid,String goodsName,Integer goodsNumber,
			Model model,HttpSession session,ModelAndView mv) {
		//获取用户信息
		User loginer = (User)session.getAttribute("loginer");
		cartService.updateNumByGoodsName(cid,goodsNumber);
		Cart cart=cartService.findCartByGoodsName(goodsName,loginer.getUserId());
	...
	public String addNewOrder(String orderName,Integer total,String userAlice,HttpSession session) {
		...
		//清空购物车
		cartService.delCart(loginer.getUserId());
	...
	// 实现购物车页面加入购物车功能
	@RequestMapping(value="/addcartcart/{goodsId}")
	public String AddCartCart(@PathVariable Integer goodsId,Model model,HttpSession session ) {
		//获取用户信息
		User loginer = (User)session.getAttribute("loginer");
		//通过goodsId获取goods
		Goods goods=cartService.findGoodsByGoodsId(goodsId);
	...
	// 实现搜索页面加入购物车功能
	@RequestMapping(value="/addcartsearch/{goodsId}")
	public String AddCartSearch(@PathVariable Integer goodsId,Model model,HttpSession session ) {
		//获取用户信息
		User loginer = (User)session.getAttribute("loginer");
		//通过goodsId获取goods
		Goods goods=cartService.findGoodsByGoodsId(goodsId);
