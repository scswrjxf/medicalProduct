            ...
            <table class="providerTable" cellpadding="0" cellspacing="0">
                ...
                <c:forEach var="category" items="${category }" varStatus="status">
					<tr>
						...
						<td>
						<span><a class="deleteCategory" href="javascript:;" categoryId=${category.categoryId } category=${category.category }><img src="${pageContext.request.contextPath }/statics/images/schu.png" alt="删除" title="删除"/></a></span>
						</td>
					</tr>
				</c:forEach>
			</table>
...
function changeDLGContent(contentStr,categoryId){
	var p = $(".removeMain").find("p");
	p.html(contentStr);
	// 把id为yes的a标签的href属性值，进行设置
	var a = $("#yes");
	a.attr("href","${pageContext.request.contextPath}/server/delcategory/"+categoryId);
}
...
$(function(){
	...
	$("body").on("click",".deleteCategory",function(){
		categoryObj = $(this);
		changeDLGContent(
				"你确定要删除分类【"+categoryObj.attr("category")+"】吗？",
				categoryObj.attr("categoryId"));
		openYesOrNoDLG();
	});
});
</script>
